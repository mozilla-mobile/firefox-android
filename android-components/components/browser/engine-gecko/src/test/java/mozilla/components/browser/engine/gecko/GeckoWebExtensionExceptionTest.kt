...
import org.mozilla.geckoview.WebExtension.InstallException.ErrorCodes.ERROR_BLOCKED
...
class GeckoWebExtensionExceptionTest {
    ...
    @Test
    fun `Handles a blocked exception`() {
        val geckoException = mock<WebExtension.InstallException>()
        ReflectionUtils.setField(geckoException, "code", ERROR_BLOCKED)
        val webExtensionException =
            GeckoWebExtensionException.createWebExtensionException(geckoException)

        assertTrue(webExtensionException is WebExtensionInstallException.Blocked)
    }
}
